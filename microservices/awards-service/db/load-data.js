...
const insertTable = `CREATE TABLE campeonatos
(
...
INSERT INTO campeonatos VALUES 
   (1, 48, 2017, 'Primero', 'Competencia de carrera', 'Estados Unidos', 10000, '*****'),
...
const loadAwards = async () => {
  try {
    await sequelize.query(`SELECT * FROM campeonatos`) ;
    console.log('Awards table already exists');
  } catch (error) {
    console.log('Awards table does not exist, creating it...');
    await sequelize.query(insertTable);
    await sequelize.query(data);
  }
};

module.exports = loadAwards;